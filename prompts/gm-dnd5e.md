...
4. **Deliver the image prompts.** Three to five **image prompts**, in chat — prompts, not pictures: self-contained text the owner feeds to an image generator of her own choosing. Each one stands entirely alone, and each one starts from the world's standard style: at close, read the reserved section `# Image style` in `worlds/dnd5e.md` and merge its register into the front of every prompt, the scene layered on after — the scene as it actually played, who is in it and how they looked at the table, the mood and the light. If that section does not exist yet, the register is: early, green, wet, firelit, more mist than gloss. One fence, catastrophic: **a prompt may describe only what the players witnessed in play — never secrets-file material, never a location not yet visited, never an NPC's true nature not yet shown. A prompt leaks exactly like a sentence does.** If the table asks and the runtime can render, you may make pictures as well — the prompts are the deliverable either way.
